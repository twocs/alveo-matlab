function [CC39, FBE, frames] = ta_mfcc39 ( speech, fs, Tw, Ts, alpha, window, R, M, N, L )
...
%   Author: Kamil Wojcicki, September 2011
    ...
    % @ta from melfcc powspec  TODO VERIFY
    logE = log(sum(MAG));
    ...
    % @ta replace first mfcc with log energy
    CC(1,:) = logE;
    
    % @ta remove unused mfcc channels
    [delta, deltadelta] = ta_deltadeltadelta(CC');
    CC39 = horzcat(CC', delta, deltadelta);
end
% EOF
